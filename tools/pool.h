...
typedef struct ALIGNED_CACHE_LINE Pool {
    ...
    size_t capacity;           // 池容量
    atomic_size_t alloc_cnt;   // 分配计数器（调试用）
    atomic_size_t free_cnt;    // 释放计数器（调试用）
    atomic_size_t contention_counter;   // 冲突计数器（调试用）
} Pool;
...
// 静态断言确保结构体对齐
static_assert(sizeof(TaggedPointer) % 16 == 0,
    "TaggedPointer structure requires 16-byte alignment");
static_assert(sizeof(Node) % CACHE_LINE_SIZE == 0,
    "Node structure requires cache line alignment");
static_assert(sizeof(Pool) % CACHE_LINE_SIZE == 0,
    "Pool structure requires cache line alignment");
